...
def main() -> int:
    ...
    lines = ["FOOTBALL SHOOT HELPER STATIC EXTRACTION", f"BINARY_SHA256: {digest}", f"METADATA: {metadata_path.as_posix() if metadata_path else 'NONE'}", ""]
    for item in resolved:
        ...
        direct: list[int] = []
        lines.append(f"### {item.target.name} {item.start:08X}-{item.end:08X} boundary={item.boundary_source} exact={'true' if item.exact_function_boundary else 'false'}")
        ...
        calls[item.target.name] = sorted(set(direct))
    ...
    print(f"SHOOT_HELPER_EXTRACTION: GREEN output={OUTPUT}")
    return 0
