...
- first-level direct callees are also emitted, capped at 48, using 0x200-byte bounded windows when an exact metadata boundary is unavailable
- an exact function boundary is claimed only when the target itself exists in `script.json` and the next-method boundary is sane/aligned; every fallback is labelled `exact=false`
